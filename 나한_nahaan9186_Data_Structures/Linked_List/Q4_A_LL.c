...
void moveEvenItemsToBack(LinkedList *ll)
{
	...
	while (cur != NULL)
	{
		...
		{
			insertNode(&temp_odd, index_odd, cur->item);
			index_odd++;
			cur = cur->next;
		}
	}
	
	LinkedList temp_even;
	temp_even.head = NULL;
	temp_even.size = 0;
	int index_even = 0;
	...
	while (cur != NULL)
	{
		...
		{
			insertNode(&temp_even, index_even, cur->item);
			index_even++;
			cur = cur->next;
		}
	}
	...
	tail->next = temp_even.head;
	if (temp_odd.head == NULL)
		ll->head = temp_even.head;
	else
		ll->head = temp_odd.head;
}
